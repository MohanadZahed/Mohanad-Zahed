...
| --------------- | --------------------- | --------------------------------- | ----------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------- |
| 0.000–0.013     | Hero                  | 4.0                               | idle drift (autonomous, no scroll needed) | centered, idle breathing, full opacity                                                                                                                                  | static, slight DoF on far logos |
| 0.013–0.027     | Hero → About          | lerp 4.0 → 1.5                    | scroll layers velocity onto idle drift    | drifts left to x ≈ −2.6, yaws 0 → ~36°; settled by progress ≈ 0.027                                                                                                     | small dolly-in                  |
| 0.027           | About pin             | 1.5                               | normal                                    | **pinned to About** in document space — anchor's world-Y now tracks scroll                                                                                              | static                          |
| 0.027–0.064     | About → Notebook      | 1.5                               | normal                                    | scrolls off-screen above with About                                                                                                                                     | static                          |
...
Implementation note: the leftward translate + yaw are driven by `smoothstep(0.41/31.25, 0.82/31.25, progress)` (clamped at 1.0). The vertical pin is `anchor.position.y = max(0, progress − 0.85/31.25) × totalScrollPx / windowHeight × viewportWorldHeight` — i.e. the anchor matches scroll velocity in world units once About is centered, so the avatar appears stationary in document space. Together these signals leave the avatar settled at the left side of the About section, and it leaves the viewport upward as the user scrolls into Notebook.
...
| Sub-progress | What happens |
| --- | --- |
| 0.00–0.667 (Phase A) | Stage is sticky-pinned at viewport top. Wheel scroll converts entirely to horizontal: the strip translates from `0` to `−X_centre`, where `X_centre` is computed at runtime so the **last** card's centre lands at the viewport centre. The header block scrolls off-screen left first; the user sees up to three cert cards at a time. |
| 0.667 (hand-off) | Sticky releases because the stage's bottom has reached the section's bottom. The page now scrolls naturally for the remaining 100vh of the section. |
| 0.667–1.00 (Phase B) | The strip keeps translating from `−X_centre` to `−X_offscreen` (last card fully off-screen left) **while** the stage scrolls upward out of the viewport in the natural document flow — so vertical scroll resumes and horizontal motion continues simultaneously. Experience begins to enter the viewport from below. |

The phase-A/phase-B split is implicit in the markup — no GSAP pin, just CSS sticky. A single section-local ScrollTrigger writes `certificatesProgress` 0..1 across both phases; the strip's translate is computed as: `p ≤ ⅔ ? lerp(0, X_centre, p / ⅔) : lerp(X_centre, X_offscreen, (p − ⅔) / ⅓)`. Below `STACK_BREAKPOINT_PX` (768 px) and under `prefers-reduced-motion`, the section degrades to a flat vertical flex-column of un-rotated cards.
