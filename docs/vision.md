...
| 0.296–0.392     | **Knowledge**         | n/a (orbit silent)                | n/a                                       | beige `--color-quaternary` backdrop (fade-in driven by global progress during late Skills), black disc falls from top and expands to a corner-filling rectangle that **persists past Knowledge into Certificates**; yoga avatar fades in, spins π → 0, continuous gentle float; five sky-blue math-pattern SVGs burst from arc centre to their upper-ellipse anchors in lockstep with the disc, rest at 0.3 opacity (cursor proximity lifts to 1) | static — drives `knowledgeProgress` |
