...
Section heights in viewport-units: Hero=1, About=1, **Manifesto=6.9** (long pinned interlude), **Skills≈2.2** (GSAP-pinned emerge intro, pulled up onto the Manifesto tail by a `-2.18` overlap margin, then natural full-height scroll), **Knowledge=3** (pinned interlude), **Certificates=3** (sticky horizontal-scroll interlude), **Experience≈16** (sticky-stacking project cards), Contact=1 → scroll range ≈ 32.77vh. The unit in source is `svh` everywhere (see `src/sections/CLAUDE.md` → Layout rules); the `vh` shorthand in this doc is purely nominal — read it as "viewport-heights" and assume `svh` at the call site. Mobile URL-bar movement does not change page heights. Section TOPS in global progress shifted with the Manifesto→Skills rework and Skills' overlap margin makes them fuzzy — treat any global-progress number in this doc as a nominal guide and use `ScrollTrigger.refresh()` measurements for anything that needs precision.
...
Behind everything sits a glowing **circuit-tree backdrop** ([src/sections/manifesto/ManifestoCircuit.tsx](../src/sections/manifesto/ManifestoCircuit.tsx)) — an inline `<svg>` of four leads dropping from the top edge of the screen and branching (orthogonal runs + 45° elbows) into a fan of filled dot nodes. It's invisible at the start, then **draws on per-trace** as a virtual sweep line falls top→bottom, over `[CIRCUIT_PAINT_START=0.045, CIRCUIT_PAINT_END=PHASE.SCALE_START=0.37]` — it begins the moment the title finishes typing and is fully drawn exactly as the notebook image starts scaling up. Recolorable via the `color` prop (`currentColor` drives strokes, dots, and glow); currently `var(--color-secondary)`. Reveal is a pure function of `progress` computed in render (no store field / `useFrame`); `prefers-reduced-motion` renders it fully drawn and static. The CSS `drop-shadow` glow is applied **only on fine-pointer (desktop) devices** — on iOS/WebKit a CSS filter rasterizes the whole animated SVG into one cached layer that fails to invalidate per-tick, leaving ghost node-dots ahead of the sweep, so coarse-pointer devices render crisp lines with no filter. As a safety net, un-revealed traces (`drawn ≤ 0`) and nodes (`op ≤ 0.01`) are not rendered at all, and node reveal is opacity-only (no SVG `transform`). It's the first child of the sticky stage (`z-index: 0`) so the laptop and the `z-30` finder boxes paint over it. Sub-phases (see [src/sections/manifesto/manifesto.constants.ts](../src/sections/manifesto/manifesto.constants.ts)):

| Sub-progress | What happens |
| --- | --- |
| 0.00–0.08 | Title `elevate your system` **types in scroll-driven** over `[0, ~0.045]` (you drive the keystrokes by scrolling), holds, then translates upward and fades as the notebook rises. The notebook image enters and by 0.08 reaches the vertical centre and pins. A muted, looping `<video>` (`/videos/notebook-video.{webm,mp4}`, **played at 2× speed**) sits *behind* the laptop image inside `SCREEN_RECT` and plays through the transparent screen cut-out — so the laptop feels alive the moment it lands. |
