...
**Notebook (section-local progress 0.00–1.00, ~6.9×viewport-tall)** — A scroll-pinned interlude. The 3D canvas is silent here; the entire scene is HTML/CSS-driven. The section-local progress is computed by the Notebook's own ScrollTrigger and passed into `NotebookStage` as a React prop (no store field). Sub-phases (see [src/sections/notebook/notebook.constants.ts](../src/sections/notebook/notebook.constants.ts)):
