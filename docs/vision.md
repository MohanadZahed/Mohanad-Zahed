...
Behind everything sits a glowing **circuit-tree backdrop** ([src/sections/notebook/NotebookCircuit.tsx](../src/sections/notebook/NotebookCircuit.tsx)) — an inline `<svg>` of four leads dropping from the top edge of the screen and branching (orthogonal runs + 45° elbows) into a fan of filled dot nodes. It's invisible at the start, then **draws on per-trace** as a virtual sweep line falls top→bottom, over `[CIRCUIT_PAINT_START=0.045, CIRCUIT_PAINT_END=PHASE.SCALE_START=0.37]` — it begins the moment the title finishes typing and is fully drawn exactly as the notebook image starts scaling up. Recolorable via the `color` prop (`currentColor` drives strokes, dots, and glow); currently `var(--color-secondary)`. Reveal is a pure function of `progress` computed in render (no store field / `useFrame`); `prefers-reduced-motion` renders it fully drawn and static. It's the first child of the sticky stage (`z-index: 0`) so the laptop and the `z-30` finder boxes paint over it. Sub-phases (see [src/sections/notebook/notebook.constants.ts](../src/sections/notebook/notebook.constants.ts)):
