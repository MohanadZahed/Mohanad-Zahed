...
Section heights in viewport-units: Hero=1, About=1, **Notebook=6** (long pinned interlude), Projects=1, Experience=1, Contact=1 → 11 viewport-heights total, scroll range = 10vh. Section TOPS in global progress: Hero=0.00, About=0.10, Notebook=0.20, Projects=0.80, Experience=0.90, Contact=1.00. About is centred in the viewport at progress = 0.10.
...
| 0.80–0.90 | Projects              | logos detach, fly to card corners | per-logo spin                             | off-screen above; pin holds                                                                                                                                             | follows scrolled cards          |
...
| 0.82–1.00 | Projects section translates upward from below the viewport via `notebookHandoff`, sliding over the still-pinned full-screen notebook like a curtain. At handoff = 1, scroll resumes inside Projects. |

**Projects (0.80–0.90)** — unknowen yet
