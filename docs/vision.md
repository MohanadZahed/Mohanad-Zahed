...
Section heights in viewport-units: Hero=1, About=1, **Notebook=6.9** (long pinned interlude), **Skills≈2.2** (GSAP-pinned emerge intro, pulled up onto the Notebook tail by a `-2.18` overlap margin, then natural full-height scroll), **Knowledge=3** (pinned interlude), **Certificates=3** (sticky horizontal-scroll interlude), **Experience≈16** (sticky-stacking project cards), Contact=1 → scroll range ≈ 32.77vh. The unit in source is `svh` everywhere (see `src/sections/CLAUDE.md` → Layout rules); the `vh` shorthand in this doc is purely nominal — read it as "viewport-heights" and assume `svh` at the call site. Mobile URL-bar movement does not change page heights. Section TOPS in global progress shifted with the Notebook→Skills rework and Skills' overlap margin makes them fuzzy — treat any global-progress number in this doc as a nominal guide and use `ScrollTrigger.refresh()` measurements for anything that needs precision.

Because Notebook (~6.9vh) spans more than one viewport, it drives its internal choreography from a section-local progress value (computed by its own ScrollTrigger and passed to `NotebookStage` as a prop — no store field). Skills uses its own section-local `useScrollStore.skillsIntro`, written by a Skills-specific GSAP-pinned ScrollTrigger; the negative `SKILLS_OVERLAP_VH` margin makes the pin engage while the user is still scrolling through the Notebook's pinned tail, so Skills emerges from the laptop screen before the notebook releases. Certificates (3vh) drives its horizontal-scroll choreography from `useScrollStore.certificatesProgress`. Global progress thresholds in scene/avatar code shift when section heights change — see "Reference scene constants" below.
...
| 0.064–0.264     | **Notebook**          | n/a (avatar gone, ring silent)   | n/a                                       | off-screen above                                                                                                                                                        | static — see Notebook storyboard |
...
**Skills (section-local `skillsIntro` 0.00–1.00, then natural scroll)** — Skills **emerges from the laptop screen**, then becomes a normal full-height scrolling section. A negative `marginTop` (`SKILLS_OVERLAP_VH` ≈ 218svh) pulls Skills up so it overlaps the notebook's pinned tail (engaging ~notebook-progress 0.80, just after `improve` fades). A GSAP `pin` (with `pinSpacing`) holds the Skills content for `INTRO_SCROLL_VH` (~118svh) of scroll while `skillsIntro` (0..1, in `useScrollStore`) drives the intro:
...
Pin releases exactly at `skillsIntro = 1` (identity), so the hand-off to **normal document flow is seamless**: the chips below the fold scroll in normally and nothing is permanently cropped. Spotlight (`CircuitBackground`) is suppressed mid-intro (the gradient lives in the scaled wrapper) and re-enabled at identity. Runs at all widths; only `prefers-reduced-motion` falls back to a plain static full-viewport section. Tuning knobs live in [src/sections/skills/skills.constants.ts](../src/sections/skills/skills.constants.ts) (`SKILLS_OVERLAP_VH`, `INTRO_SCROLL_VH`, `EMERGE_END`, `ZOOM_START`, `ZOOM_END`) and the rect in `SKILLS_SCREEN_RECT` ([notebook.constants.ts](../src/sections/notebook/notebook.constants.ts)).
