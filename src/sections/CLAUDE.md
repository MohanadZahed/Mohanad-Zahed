...
| 0.064–0.264 | Notebook | 3D canvas is silent. Section is fully HTML/CSS and uses `useScrollStore.notebookProgress` (section-local, 0..1) for its own choreography. A looping muted `<video>` (`/videos/notebook-video.{webm,mp4}`) plays behind the laptop image inside `SCREEN_RECT`; over `[TERMINAL_OPEN_START=0.32, TERMINAL_OPEN_END=0.36]` a black "terminal" rectangle scales out of the screen's bottom-right corner (`transformOrigin: bottom right`, `scale(0 → 1)`) to cover it. Once covered, the video is paused; the rectangle is sized in `%` of `SCREEN_RECT` so it scales with the notebook during `SCALE_START → SCALE_END` for free. |
